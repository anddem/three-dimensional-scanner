...
class ToupCamCamera(object):
    ...
    def get_jpeg_data(self, data=None, quality=100):
    ...
    def get_camera(self, cid: str=None):
        func = lib.Toupcam_Open
        func.restype = ctypes.POINTER(HToupCam)
        cam = func(cid.encode('ascii'))
        return cam
...
if __name__ == '__main__':
    t_cams = toupcam.Toupcam.EnumV2()
    print(t_cams)
    cameras = [ToupCamCamera(size=(2048, 1526), cid=t_cam.id) for t_cam in t_cams]
    for camera in cameras:
        camera.open()
        camera.set_auto_exposure(True)
        print(camera.get_temperature_tint())
    import time
    time.sleep(1)
    print('ready')
    while True:
